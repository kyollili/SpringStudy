...
public class EmpDAO {
	 ...
	 * 			BeanFactory : DI(core)
	 * 				|
	 * 			ApplictaionContext : AOP => AnnotationConfigApplicationContext(자바로 환경설정이 된 경우)
	 * 				|
	 * 			WebApplicationContext : MVC
	 * 	****page5 => 최신의 스프링은 자바 기반의 환경설정을 많이 사용하고 있다
	 * 		=====> 환경설정 파일 : 생성, 주입, 클래스간의 연결 관계 설정
	 * 									---------------- XML,자바 => 스프링에서 읽어서 설정
	 * 									---------------- XML은 반드시 스프링에서 지원하는 태그와 속성만 사용이 가능
	 * 		객체생성
	 * 		-----
	 * 		1. 클래스 한 개씩 메모리 할당 <bean>
